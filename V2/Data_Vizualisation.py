...
def Data_Vizualisation(DataFrame,Confidence_Level,periods_SMA,periods_EMA,DataFrame_Amount_Each_Value) :
    ...
    def Portfolio_Value_At_Risk_plot(DataFrame, Confidence_Level,ax) :
        Value_At_Risk = round(Calc_VaR_Single(DataFrame["Portfolio"], Confidence_Level),5)
    ...
    def Portfolio_RSI_plot(DataFrame, ax):
        RSI_portfolio(DataFrame)
        ax.set_title("RSI_14")
        ax.plot(DataFrame.index[14::], DataFrame["RSI14"].iloc[14::], label="RSI" )
        ax.axhline(70,color='red',linestyle="--",label="Zone de surachat")
        ax.axhline(30,color='green',linestyle="--",label="Zone de survente")
        ax.legend()
        
    def Portfolio_distribution(DataFrame_Amount_Each_Value,ax):
        ax.axis('off')
        table = ax.table(
            cellText=DataFrame_Amount_Each_Value.values,
            colLabels=DataFrame_Amount_Each_Value.columns.tolist(),
            rowLabels= DataFrame_Amount_Each_Value.index.tolist(),
            loc='center'
        )
    ...
    def plot_graph(periods_SMA,periods_EMA) :
        ...
        Portfolio_Value_At_Risk_plot(DataFrame, Confidence_Level,axes[0][0])
        Portfolio_SMA_plot(DataFrame, periods_SMA, axes[0][1])
        Portfolio_EMA_plot(DataFrame, periods_EMA, axes[1][0])
        #Portfolio_RSI_plot(DataFrame,axes[1][1])
        Portfolio_distribution(DataFrame_Amount_Each_Value,axes[1][1])
        fig.autofmt_xdate()
